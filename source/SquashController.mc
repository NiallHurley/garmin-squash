...
class SquashController
{
    var mTimer;
    var squashModel;
    var mRunning; // are we running or not: Boolean
    ...
    function initialize() {
        ...
        squashModel = Application.getApp().model;
    ...
    function save() {
        ...
        WatchUi.pushView(new WatchUi.ProgressBar("Saving...", null), new SquashProgressDelegate(), WatchUi.SLIDE_DOWN);
    ...
    function discard() {
        ...
        // and start a timer to allow all processing to finish
        WatchUi.pushView(new WatchUi.ProgressBar("Discarding...", null), new SquashProgressDelegate(), WatchUi.SLIDE_DOWN);
        mTimer = new Timer.Timer();
        mTimer.start(method(:onExit), 3000, false);
    }
    ...
    function getTime() {
        return squashModel.getElapsedTimeInSeconds();
    }
    ...
    function onStartStop() {
        if(mRunning) {
            stop();
            WatchUi.pushView(new Rez.Menus.MainMenu(), new SquashMenuDelegate(), WatchUi.SLIDE_UP);
        } else {
            start();
        }
    }
